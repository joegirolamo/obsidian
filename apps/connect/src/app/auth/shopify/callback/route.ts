...
export async function GET(request: Request) {
  try {
    ...
    const tokenResponse = await fetch(`https://${process.env.SHOPIFY_SHOP_NAME}.myshopify.com/admin/oauth/access_token`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.redirect(
      new URL(`/connect/${businessCode}?success=Shopify connected successfully`, request.url)
    );
  } catch (error) {
